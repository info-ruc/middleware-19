package com.zoneyet.robot.admin.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public ModelAndView add()throws Exception{
		...
		try{
			...
			pd.put("createtime", new Date());	//查看权限
			roleService.add(pd);
			FHLOG.save(Jurisdiction.getUsername(), "新增角色:"+pd.getString("ROLE_NAME"),super.getIP());
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public ModelAndView edit()throws Exception{
		...
		try{
			pd = this.getPageData();
			/*if("robotadmin".equals(Jurisdiction.getUsername())&&pd.getString("ROLE_ID")||"admin".equals(Jurisdiction.getUsername())){*/
			    roleService.edit(pd);
			
			FHLOG.save(Jurisdiction.getUsername(), "修改角色:"+pd.getString("ROLE_NAME"),super.getIP());
	...
	public Object deleteRole(@RequestParam String ROLE_ID)throws Exception{
		...
		try{
			...
			if(roleList_z.size() > 0){
				...
				FHLOG.save(Jurisdiction.getUsername(), "删除角色ID为:"+ROLE_ID,super.getIP());
				errInfo = "success";
				}
			}
		} catch(Exception e){
	...
	public void saveMenuqx(@RequestParam String ROLE_ID,@RequestParam String menuIds,PrintWriter out)throws Exception{
		...
		FHLOG.save(Jurisdiction.getUsername(), "修改角色菜单权限，角色ID为:"+ROLE_ID,super.getIP());
		PageData pd = new PageData();
		try{
	...
	public void saveB4Button(@RequestParam String ROLE_ID,@RequestParam String menuIds,@RequestParam String msg,PrintWriter out)throws Exception{
		...
		FHLOG.save(Jurisdiction.getUsername(), "修改"+msg+"权限，角色ID为:"+ROLE_ID,super.getIP());
		PageData pd = new PageData();
		pd = this.getPageData();
		try{
			if(null != menuIds && !"".equals(menuIds.trim())){
